...
public class PageBoundsPlugin extends PluginAdapter {
    ...
    public PageBoundsPlugin() {
        rowBounds = new FullyQualifiedJavaType("com.github.miemiedev.mybatis.paginator.domain.PackageEnum"); //$NON-NLS-1$
    ...
    private void copyAndAddMethod(Method method, Interface interfaze) {
        ...
        method.getParameters().get(0).addAnnotation("@Param(\"param\")");
        newMethod.setName(method.getName() + "WithPageBounds"); //$NON-NLS-1$
        Parameter pageBounds = new Parameter(rowBounds, "pageBounds");
        pageBounds.addAnnotation("@Param(\"page\")");
        newMethod.addParameter(pageBounds); //$NON-NLS-1$
